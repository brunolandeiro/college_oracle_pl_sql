SPOOL BRUNO_PROJECT_PART_6_Q1_SPOOL.txt
SELECT TO_CHAR(SYSDATE, 'DD Month Year Day HH:MI:SS Am') from dual;
SET SERVEROUTPUT ON;
-- Question 1:
-- Run script 7northwoods in schemas des03
-- Create a procedure to display all the faculty member (f_id, f_last, f_first,  f_rank), 
-- under each faculty member, display all the student advised by that faculty member (s_id, s_last, s_first, birthdate, s_class).
