SPOOL BRUNO_PROJECT_PART_6_Q3_Q4_SPOOL.txt
...
CURSOR ITEM_CURR IS SELECT ITEM_ID, ITEM_DESC, CAT_ID FROM ITEM;
CURSOR INV_CURR(P_ITEM_ID INVENTORY.ITEM_ID%TYPE) IS 
  SELECT INV_ID, COLOR, INV_SIZE, INV_PRICE, INV_QOH FROM INVENTORY WHERE ITEM_ID = P_ITEM_ID;
BEGIN 
  FOR OBJ_ITEM IN ITEM_CURR LOOP
    DBMS_OUTPUT.PUT_LINE('');
    DBMS_OUTPUT.PUT_LINE('---ITEM---');
    DBMS_OUTPUT.PUT_LINE(OBJ_ITEM.ITEM_ID ||', '|| OBJ_ITEM.ITEM_DESC ||', CAT_ID: '|| OBJ_ITEM.CAT_ID);
    DBMS_OUTPUT.PUT_LINE('---INVENTORY---');
    FOR OBJ_INV IN INV_CURR(OBJ_ITEM.ITEM_ID) LOOP
...
CURSOR ITEM_CURR IS 
  SELECT ITEM.ITEM_ID, ITEM.ITEM_DESC, ITEM.CAT_ID, SUM(INV_PRICE * INV_QOH) "VALUE"
    FROM ITEM JOIN INVENTORY ON INVENTORY.ITEM_ID = ITEM.ITEM_ID 
    GROUP BY ITEM.ITEM_ID, ITEM.ITEM_DESC, ITEM.CAT_ID;
